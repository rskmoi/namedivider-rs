...
fn create_divider(mode: &String) -> Box<dyn NameDivider>{
    if mode == "basic" {
        let basic_divider = get_basic_name_divider(
            " ".to_string(),
            true,
        ...
        let gbdt_divider = get_gbdt_name_divider(
            " ".to_string(),
            true,
            "kanji_feature".to_string());
...
fn main() {
    ...
    match cli.action {
        Action::Name { undivided_name , mode} => {
            let divider = create_divider(&mode);
            let divided_name = divide_name(&divider, &undivided_name);
            println!("{}{}{}", divided_name.family, divided_name.separator, divided_name.given);
        },
        Action::File {undivided_name_text, mode} => {
        ...
        Action::Accuracy {divided_name_text, mode} => {
            ...
            for divided_name in file_contents.lines(){
                ...
                let divided_name = divide_name(&divider, &undivided_name.to_string());
                let divided_name_str = divided_name.family + &divided_name.separator + &divided_name.given;
                total += 1.0;
