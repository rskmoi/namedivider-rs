...
async fn health_check() -> impl Responder {
    HttpResponse::Ok().json(HealthStatus{health: "OK".to_string()})
}
...
async fn validate(division_request: &web::Json<DivisionRequest>) -> Result<(), HttpResponse> {
    if !["basic", "gbdt"].contains(&division_request.mode.as_str()){
        ...
        return Err(HttpResponse::UnprocessableEntity().json("You can only divide up to 1000 names at a time."));
    }
    Ok(())
}
...
fn get_divider(mode: &String) -> Box<dyn NameDivider> {
    if mode == "basic" {
        ...
        let divider = get_gbdt_name_divider(
            " ".to_string(),
            true,
            "gbdt".to_string());
        return Box::new(divider);
