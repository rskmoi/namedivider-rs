...
impl UndividedNameHolder{
    pub fn new(original_name: String)-> Self{
        ...
        Self{ original_name, normalized_name}
    }
...
impl NameDividerBase{
    ...
    fn create_divided_name(&self, family: String, given: String, score: f64, algorithm: String) -> DividedName{
        DividedName{
            family,
            given,
    ...
    fn divide_by_rule_base(&self, undivided_name: &String) -> Option<DividedName>{
        ...
        if fullname_length == 2{
            return Some(self.create_divided_name(undivided_name.chars().next().unwrap().to_string(),
        ...
        for (i, c) in undivided_name.chars().enumerate(){
            ...
            if i >= 2 && is_kanji_list[0] != is_kanji && is_kanji_list[is_kanji_list.len() - 2] == is_kanji {
                let (family, given) = slice(undivided_name, i - 1);
                return Some(self.create_divided_name(family,
                                                     given,
                                                     1.0,
                                                     "rule".to_string()));
            }
    ...
    fn divide_by_algorithm(&self, undivided_name:&String, score_calculator: &impl ScoreCalculator) -> DividedName{
        ...
        self.create_divided_name(pred_family,
                                        pred_given,
                                        score,
                                        self.algorithm_name.clone())
    }

    fn _divide_name(&self, undivided_name: &String, score_calculator: &impl ScoreCalculator) -> DividedName{
    ...
    pub fn divide_name(&self, undivided_name: &String, score_calculator: &impl ScoreCalculator) -> DividedName{
